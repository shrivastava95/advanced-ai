...
### ------- You can make changes to this file from below this line --------------
    ...
    def train(self,batch_size):
        ...
        expected_q_value = rewards + self.discount * next_q_value
    ...
    def GetReward(self, new_state, state, obs):
        ...
        if obs == game_state.GameObservation.Reached_Goal:
            ...
            safe_dist = (game_constants.ENEMY_SIZE + game_constants.PLAYER_SIZE) * 2
    ...
    def TrainModel(self):
        ...
        for _ in range(epochs):
            ...
            for __ in range(max_steps_per_epoch):
                ...
                self.remember(self.GetGoodState(og_state), action_num, reward, self.GetGoodState(state))
                avg_loss += self.train(batch_size)

                if obs == game_state.GameObservation.Enemy_Attacked or obs == game_state.GameObservation.Reached_Goal:
                    break
            ...
            avg_loss /= steps
            print("Epoch: [{}/{}]\t| Total Reward: {}\t| Average Loss: {:.3f}".format(_,epochs,total_reward,avg_loss))
